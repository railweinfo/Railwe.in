...
// (pre-filled amount + payee — this only OPENS the user's UPI app, it
// cannot confirm the payment happened) and an "I've paid" button that
// writes a claim doc to Firestore for the admin to verify manually and
// then approve from admin.html. This intentionally does NOT auto-upgrade
// anyone to 'paid' — see firestore.rules, which only lets the admin
...
function buildUpiLink() {
  const params = new URLSearchParams({
    ...
    am: UPI_AMOUNT,
    cu: 'INR',
    tn: 'NDA Register lifetime activation'
  });
...
async function submitPaymentClaim() {
  const btn = document.getElementById('payClaimBtn');
  const status = document.getElementById('payClaimStatus');
  if (!currentUser) return;
  btn.disabled = true;
  status.classList.remove('error');
  status.textContent = 'Sending\u2026';
  try {
    await addDoc(paymentClaimsRef(), {
      uid: currentUser.uid,
    ...
    status.textContent = 'Thanks — the admin will verify your payment and activate your account shortly.';
  } catch (err) {
    console.error('Could not submit payment claim:', err);
    status.classList.add('error');
    status.textContent = 'Could not send this. Please try again.';
    btn.disabled = false;
  }
}
...
function wirePaymentWidget() {
  ...
  const payClaimBtn = document.getElementById('payClaimBtn');
  const payPanelCloseBtn = document.getElementById('payPanelCloseBtn');

  payUpiLink.href = buildUpiLink();
  ...
  payClaimBtn.addEventListener('click', submitPaymentClaim);
}
