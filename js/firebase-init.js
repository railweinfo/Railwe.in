...
function showGateBlocked(message) {
  ...
  if (gateUpiLink) {
    gateUpiLink.dataset.upiUrl = buildUpiLink(currentUser ? currentUser.email : null);
  }
}
...
function showPaymentWidget(show) {
  ...
  if (show && currentUser) {
    ...
    if (payUpiLink) payUpiLink.dataset.upiUrl = buildUpiLink(currentUser.email);
  }
}
...
function wirePaymentWidget() {
  ...
  // Navigate directly via window.location instead of an <a target="_blank">
  // link — see the matching comment on gateUpiLink in wireUpUI() for why.
  payUpiLink.addEventListener('click', () => {
    const url = payUpiLink.dataset.upiUrl;
...
function wireUpUI() {
  ...
  // Navigate directly via window.location instead of an <a target="_blank">
  // link — custom URI schemes like upi:// are unreliable through
  // target="_blank" on mobile (some browsers open/close a blank tab during
  // the app handoff, which can look like the page "reopened" or reset).
  // Direct navigation hands off to the UPI app cleanly and this page stays
  // exactly as it was underneath.
  gateUpiLink.addEventListener('click', () => {
    const url = gateUpiLink.dataset.upiUrl;
    if (url) window.location.href = url;
  });
