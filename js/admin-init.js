...
async function loadUsers() {
  ...
  try {
    ...
    // ---- TEMPORARY DEBUG: persistent raw state, shown above the list ----
    let debugBox = document.getElementById('rawDebugBox');
    if (!debugBox) {
      debugBox = document.createElement('p');
      debugBox.id = 'rawDebugBox';
      debugBox.style.cssText = 'text-align:left; font-size:10px; word-break:break-all; color:#b3392c; background:#fbeeec; border:1px solid #e6b8ae; border-radius:4px; padding:8px; margin-bottom:10px;';
      list.parentNode.insertBefore(debugBox, list);
    }
    debugBox.textContent = `RAW DEBUG: snap.size=${snap.size}, users.length=${users.length}, adminFilter="${adminFilter}", cache=${JSON.stringify(adminUsersCache.map(u => ({uid: u.uid, email: u.email, status: u.status})))}`;
    // ---- END TEMPORARY DEBUG ----
